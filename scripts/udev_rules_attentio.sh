...
# Attentio USB Device (VID:PID from pid.codes)
SUBSYSTEM=="usb", ATTRS{idVendor}=="1209", ATTRS{idProduct}=="eea1", MODE="0666", ENV{ID_MM_DEVICE_IGNORE}="1"
SUBSYSTEM=="tty", ATTRS{idVendor}=="1209", ATTRS{idProduct}=="eea1", MODE="0666", SYMLINK+="attentio-%s{serial}"

# Attentio Bootloader fallback (STM32 DFU - blank/erased board with no valid app header)
